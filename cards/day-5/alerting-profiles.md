...
- Davis строит baseline автоматически (референсный период 7 дней) и ловит статистически значимые отклонения; delay 3-5 минут перед первым уведомлением гасит проблему, мигнувшую за 90 секунд.
...
- ЕСЛИ в maintenance window подавить только уведомления → обнаружение работает, гаснут лишь нотификации; ЕСЛИ отключить и детекцию → проблемы за окно не отображаются вовсе.
...
Metric events жив под SaaS-баннером (баннер вслух не читаю): показываю колонки списка (Enabled / Summary / Query type / Model type / Management zone) и блок Overview of limits со строками «N out of 100» по типам модели (сверено 2026-07-28). Дальше разбираю документированный лимит 10 000 конфигураций на environment и типовой кейс бизнес-метрики (меньше 10 заявок на кредит за минуту в рабочие часы), конкретные пороги тенанта по памяти не называю.
Лекторский сценарий: workshop/day-3.md, блок [1:50-2:05] «Кастомное оповещение: три простых шага». <!-- qc:ignore=CARD_UNSOURCED_NUMBER -->
...
- «Как не будить дежурного на плановые работы?» Ответ: оформить maintenance window: можно погасить только нотификации либо вместе с самим обнаружением проблем; период окна исключается из расчёта baseline.
- «Куда сегодня прилетают ваши алерты и кто их читает?» Ответ на обсуждение: если честный ответ «в общий ящик, никто», это первый кандидат на переделку: цепочка «событие → профиль → интеграция» должна заканчиваться на конкретном канале конкретной команды.
