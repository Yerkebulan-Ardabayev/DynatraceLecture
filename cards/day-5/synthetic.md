...
- ЕСЛИ защищённый endpoint штатно отдаёт 401 → в настройках самого HTTP-монитора 401 назначают допустимым кодом ответа, иначе живой сервис будет считаться упавшим (env-страница outage handling этим не управляет).
