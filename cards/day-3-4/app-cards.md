...
- «Какой порог T поставить внутреннему порталу?» Ответ на обсуждение: внутренние системы обычно жёстче публичных (сотрудник работает в системе весь день, задержка бьёт по операционке), публичным даётся мягче; главное, чтобы порог был решением команды, а не дефолтом «потому что так стояло».
