...
## ЗАПАСНОЙ ПЛАН
В air-gapped feed уязвимостей прокидываю через разрешённый прокси к Mission Control / Cloud Control либо организую оффлайн-импорт по согласованному регламенту; без свежего feed RVA продолжает работать на текущих данных, новые CVE подхватятся после следующего обновления.
...
- «Откуда база уязвимостей в закрытом контуре?» Ответ: Dynatrace Vulnerability feed плюс NVD через Mission Control / Cloud Control по контролируемому каналу, в air-gapped через разрешённый прокси или оффлайн-импорт.
