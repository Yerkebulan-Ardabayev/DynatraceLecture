...
## Сессия 2026-07-17 — air-gapped аудит (Mission Control / доставка пакетов / customer portal)

Триггер: независимый аудит «Managed закрытый контур 100%» (субагент SaaS/air-gap + ручная сверка живой докой). Найдено: канал доставки CVE-feed и вся схема скачивания пакетов описывались через недокументированный «customer portal».

| Блок | Живой источник (2026-07-17) | Решение |
|---|---|---|
| appsec: «feed через Cloud Control / Mission Control, доезжает за ~2 часа, сверка раз в минуту, состав OSV.dev/GitHub/NVD/vendor/Snyk/research» | https://docs.dynatrace.com/managed/secure/application-security/vulnerability-analytics: «Your Dynatrace Managed cluster needs to be connected to Mission Control» + endpoint `https://mcsvc.dynatrace.com/vulnerabilityFeed/*`; https://docs.dynatrace.com/managed/secure/faq: «either the Dynatrace Vulnerability feed or NVD, depending on the vulnerable component». Таймингов и состава feed на живых страницах больше нет | ⚠️ переписан: Mission Control + endpoint + «Dynatrace Vulnerability feed либо NVD»; тайминги «2 часа / раз в минуту», состав feed и «заменил Snyk» удалены (в живой доке отсутствуют) |
| appsec air-gapped: «оффлайн-импорт по согласованию с Dynatrace» | Ни managed/secure/faq, ни vulnerability-analytics офлайн-механизм не описывают; two candidate-страницы (third-party-vulnerability-feeds, vulnerability-evaluation) = 404 | ❌ удалён как недокументированное обещание; заменено на честное «офлайн-механизм доставки feed в публичной документации Managed не описан» (+ карточка appsec, 2 места) |
| problems-feature: «AutomationEngine в air-gapped доступен ограниченно» | Workflows/AutomationEngine в /managed/ отсутствует; остальные 4 места курса говорят «не активен, часть Apps-платформы SaaS» | ⚠️ приведено к «в Managed не активен» (консистентно с alerting-logic/architecture) |
| Обновления кластера: «customer portal (customers.dynatrace.com), раздел загрузок, все версии за всё время контракта», «полный пакет (server+OneAgent+AG+manifest)», CMC «Cluster updates → Upload package / Maintenance → Update», «.tar.gz», «CMC валидирует SHA256/подпись» | https://docs.dynatrace.com/managed/managed-cluster/operation/update-dynatrace-managed: automatic / semi-automatic («This email contains links to download the installation packages») / manual; UI «Settings > Automatic update» → «Upload installation package»; директории `/opt/dynatrace-managed/installer/upgrade` (кластер) и `/var/opt/dynatrace-managed/agents` (OneAgent, RUM JavaScript, ActiveGate, Synthetic). Customer portal на странице не упоминается; dtkb по официальному зеркалу: «customer portal» = 0 хитов | ⚠️ architecture.md (Шаги 1/3, чек-лист release notes) и components.md (2 схемы обновления) переписаны на email-ссылки + Settings → Automatic update + раздельные пакеты + документированные директории; выдуманное имя файла `dynatrace-managed-cluster-update-X.Y.Z.tar.gz` удалено; карточка architecture обновлена |
| Установка: «customers.dynatrace.com → Downloads → Dynatrace Managed Installer, LTS, ~2 ГБ», «.lic скачивается с customer portal» | https://docs.dynatrace.com/managed/managed-cluster/installation/dynatrace-managed-offline: «Download the following files from the activation email…: License file (license.lic). Dynatrace Managed installer (cluster installer). OneAgent, RUM JavaScript, ActiveGate, and Synthetic installation packages (agents bundle)»; установка `sudo /bin/sh dynatrace-managed-<version>.sh --license-file license.lic`; активация лицензии в CMC файлом `license-activated.lic` | ⚠️ Шаг 1 переписан на активационное письмо; команда установки дополнена `--license-file license.lic`; «около 2 ГБ» и «LTS» удалены; активация лицензии = license-activated.lic |
| Источники образов/SDK «с customer portal» (kubernetes, containers, oneagent-infra extensions, instrumentation Mobile SDK, data-explorer биллинг) | Официальные доки канал «customer portal» не документируют; Mobile: Android Gradle-плагин в Maven Central, iOS через Swift Package Manager (подтверждено аудитом Day 5) | ⚠️ все 6 мест переписаны: без имени канала (образы/extension качаются «на машине с интернетом») либо на документированные источники (Maven Central / SPM); карточка data-explorer переформулирована |

Гейты после правок: quality_check --cards ✅ 0 issues; em-dash 0; customer portal / Cloud Control / оффлайн-импорт: 0 вхождений.

## Сессия 2026-07-17 (продолжение) — независимый аудит day-2 и day-3-4 (субагенты + ручная сверка)

Волна read-only субагентов по дням, каждое спорное число перепроверено главным потоком по живым /managed/-страницам. Ключевой дрейф доков с апреля: retention и список интеграций.

| Блок | Живой источник (2026-07-17) | Решение |
|---|---|---|
| Retention: «RUM User action data до 35 дней», «Log Monitoring Classic 35 дней (фикс)» (oneagent-infra, mlt-concepts + карточки) | shortlink/data-retention-periods: «User action data: Configurable, with maximum 365 days»; «Log Monitoring: Configurable, with maximum 90 days»; 35 дней = User sessions (fixed) | ⚠️ переписано: user action data до 365, логи настраиваются до 90 (5 мест в 2 explanations + 4 места в 2 карточках; CARD-гейт поймал 2 хвоста) |
| Problem notifications: «ChatOps: Slack, Microsoft Teams» (incident-lifecycle) | Живой список Managed: Opsgenie, PagerDuty, VictorOps, xMatters, Jira, Slack, ServiceNow, Email, Webhook, Trello. Teams отсутствует (в апрельском аудите был) | ⚠️ Teams убран из готовых интеграций (упомянут как случай для Webhook), Trello добавлен |
| Metric selector: `builtin:…:filter(dim=="значение")` (kubernetes) | metric-v2/metric-selector: условия только функциями eq()/ne()/in() и т.п., оператор `==` невалиден | ⚠️ пример переписан на `:filter(eq(k8s.namespace.name,"retail-prod"))` |
| «nginx-ingress распознаны как Web service» (services-overview) | service-types: Web request services «serve the web applications… (Apache, IIS, NGINX)»; Web service = только WSDL | ⚠️ исправлено на Web request service |
| SDv2: «включается там, где доступно по версиям OneAgent», «на странице список v2-правил и статус парка», кукбук «Enable v2 detection for OneAgent 1.XXX+» (services-overview) | service-detection-v2: «SDv2 rules apply only to OpenTelemetry services and the Adobe Experience Manager»; захват страницы: один тумблер | ⚠️ переписано: тумблер без списка/статуса; кукбук-сценарий переделан под OpenTelemetry; переходный период через версии агентов удалён |
| Пути меню: «Settings → Server-side service monitoring → …» (services-overview, service-cards + карточки) | Захват sidebar: раздел называется «Service Detection» | ⚠️ 6 путей исправлены |
| Колонки Problems: «Impacted = количество, Affected = главная сущность» (problems-navigation + карточка) | Захват строки списка: имя сущности стоит в Impacted, число в Affected; собственный пример курса противоречил его же определениям | ⚠️ значения колонок поменяны местами, примеры приведены в соответствие |
| `builtin:os-services.*` (os-monitoring) | os-services: метрика называется `builtin:osservice.availability` | ⚠️ исправлено |
| Экраны настроек: «Ignore patterns / Deep monitoring options» (containers), поля подключения на Monitoring settings (kubernetes), «правила включения / лимит 100 / naming rules» на Endpoint metrics (service-cards) | Захваты страниц: только тумблеры (и Filter items); исключения контейнеров = отдельная страница Container monitoring rules | ⚠️ описания экранов приведены к захвату, выдуманные поля удалены, концепты перенесены в правильные места |
| Недокументированные детали: t-digest, Total/Average time и «самый медленный = суммарное время» (Top DB statements), строка «Пул соединений» как видимая клиентской инструментацией, свойства Process grouping rules (K8s label / Docker image / path / JVM arg), 4 типа правил Full Web Services, операторы и «порты 80/443» и «в течение минуты» в app-detection, crash rate «окно 10 мин / 10 юзеров», Go 1.323+, JNDI в code-level, только Windows/Linux для всех RAP-стеков | В живой документации отсутствуют; документированные замены: Top statements сортируется по Response time / Fetch count / Row count; process grouping = Simple (env var / Java system property, split-only) + Advanced (split/merge); code-level классы = SQL injection / command injection / improper input validation / SSRF; RAP = .NET Framework 4.5+ и .NET Core 3.0+, ограничение Windows x86/Linux x86 в доке привязано к Java | ⚠️/❌ все перечисленные детали удалены или заменены документированными формулировками (9 файлов + 6 карточек) |
| Severity-корзины Security overview: «Critical / High / Medium / Low» | Захват /ui/security/overview: 35Critical / 237High / 368Medium / 73Low / 15None | ⚠️ добавлена корзина None |
| Web Vitals в instrumentation: «LCP, FID, CLS»; iOS «SPM или CocoaPods» | INP заменил FID (внутренняя консистентность курса + правило); публикация CocoaPods остановлена | ⚠️ FID→INP; CocoaPods помечен остановленным |

Не тронуто осознанно: «In development»-скрины reliability-config (исторический захват со скриншотами, на Mac скриншоты отсутствуют, ui_elements контент-зону Settings не видит); Elasticsearch replication factor 2 и прочие «мелочи без вердикта» субагентов (не подтверждены и не опровергнуты, вынесены сюда как кандидаты следующего прохода).

Гейты: quality_check --cards ✅ 0 issues (CARD_UNSOURCED_NUMBER поймал 2 хвоста «35» в карточке mlt-concepts, исправлены); em-dash 0; v1+v2 пересобраны.

## Сессия 2026-07-17 (финал) — независимый аудит day-1, day-5 и языковой проход

Завершение волны: day-1 (34 finding), day-5 (24), лингвист (32 позиции). Каждое спорное число главный поток перепроверял по живым /managed/-страницам; захват ui_elements.json использован как ground truth экранов.

Ключевые фактические правки day-1: HU-лестница дополнена дробными ступенями 0.10/0.25/0.50 (доке противоречило «до 16 GiB → 1.0» как минимум); retention в architecture (логи до 90, synthetic до 365); каденция релизов «6-8 недель» заменена фактом из release notes (весна 2026: примерно раз в месяц); Cluster ActiveGate ставится отдельно (не «встроенный один»); Windows-ребут для OneAgent не требуется; Discovery-режим помечен как DPS-only; диск ActiveGate приведён к цифрам доки; варианты Target version = 4 (добавлен Specific version); recurrence окон Once/Daily/Weekly/Monthly (разовое окно возможно); пути исправлены (Settings → Updates; Maintenance windows верхнего уровня; Deployment Status → ActiveGates; карточка хост-группы вместо несуществующей категории Host groups); Smartscape жив (404 был на Apps-роуте демо); пустой экран Frontend объяснён заглушкой «Too many requests», а не «RUM не настроен»; колонки списка сессий приведены к захвату; Impacted/Affected поменяны местами и в problems-feature; недокументированные детали удалены или помечены (роли IAM как иллюстрация, тайминги 5-15 с, лицензионные веса, systemd-юниты, флаги добавления узла, custom.properties, N/N+1, Acknowledge-кнопка, «5 минут стабильности», «-javaagent» заменён на JVMTI, AIX-задержка, Extensions «с 2024»).

Ключевые правки day-5: токены приведены к двухслойной доке (пример авторизации dt0c01, таблица префиксов dt0s01; dt0s02/dt0s16 = облачная платформа, в air-gapped не используются) и в explanation, и в карточке; семантика Allow list / Block list в Session Replay была инвертирована, исправлена по дословным цитатам configure-session-replay-web; выдуманные поля страниц-тумблеров удалены (cookie-настройки SR, availability settings и outage-пороги synthetic, поля Token settings, cost control лимиты, пороги UX score на env-странице, rage/dead/error/slow цифры); beacon идёт на endpoint Cluster ActiveGate /bf/, а не в CMC; UX score = Satisfying/Tolerable/Frustrating; USQL-пример перестал приравнивать распределение UX score к Apdex; Teams удалён из готовых интеграций (подключение через Custom webhook), Trello добавлен; пути Manage → System Notifications / Access Tokens и Settings → Cloud Automation → Issue-tracking; «два независимых тумблера» maintenance windows заменены на документированный «режим подавления» (4 места); «3 раза подряд» retry и «первые 500 сессий» удалены как несорсированные.

Языковой проход: устранён смешанный язык и гомоглифы (trackerom, factа, finaл, AI-engine, Per-* связки), обрывки после вычистки тире, грубые падежи; глоссы унифицированы (профиль алертинга, обнаружение аномалий, карта топологии, граф вызовов сервиса, трейсы вместо трасс и др.). Решения нормы за владельцем (см. отчёт лингвиста): endpoint vs эндпоинт, environment/окружение/тенант, alert/уведомление/оповещение, блок «Термины темы» для day-1, апострофные формы (endpoint'ов) в CONTENT_POLICY.

Гейты финала: quality_check --cards ✅ 0 issues (CARD_UNSOURCED_NUMBER дважды ловил хвосты карточек при снятии цифр из explanations, все добиты); em-dash 0; link_check 149×200 (добавлен источник os-services); v1+v2 пересобраны. Полные таблицы findings пяти аудиторов сохранены в выводах субагентов сессии 2026-07-17.
