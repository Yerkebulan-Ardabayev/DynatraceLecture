> 📅 **День 1: Введение в систему Dynatrace** → Тема 10 из 14: «Функционал Problems: автоматическая детекция инцидентов»
...
**Окно времени.** Кнопка **Edit timeframe** в верхней панели. Типовая ошибка дежурного: смотреть только открытые проблемы за последние 2 часа; ночная проблема к утру закрылась и «исчезла». Приём: расширить окно до 72 часов и переключить Status на Closed: проблема найдётся.

**Слева: панель фильтров.** Открывается кнопкой **Toggle Global Filter panel**, заголовок панели **Filter by** (названия сверены на живом тенанте 2026-07-28). Секции:
...
1. Открыть **Observe and explore → Problems** (`/ui/problems`).
2. Кнопкой **Edit timeframe** расширить окно до 72 часов.
3. В панели **Filter by** (кнопка Toggle Global Filter panel) поставить Status = Closed и найти любую проблему типа Slowdown.

Что должно получиться: конкретная закрытая Slowdown-проблема с её `P-NNNNNNNN`-идентификатором; заодно закреплён приём «ночная проблема исчезла → расширить окно и включить Closed».
