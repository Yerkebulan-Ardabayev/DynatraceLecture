...
>
> 🔖 **Редакция от 2026-04-26.** Тех-факты сверены с docs.dynatrace.com (Smartscape — общая концепция OneAgent для Managed и SaaS, реализация 1:1; в air-gapped Managed работает на данных OneAgent + ActiveGate без выхода в интернет). Все ссылки проверены `scripts/link_check.py`. <!-- revision: 2026-04-26 -->
...
Эти данные OneAgent отправляет в кластер через ActiveGate. Кластер собирает данные со всех хостов в единый граф и обновляет его постоянно: новое соединение появляется на карте сразу, исчезнувшее через 30 минут без активности помечается устаревшим.
...
**Горизонтальные связи** — вызовы между сущностями одного уровня. Самое важное — между сервисами: PaymentService вызывает AccountService, AccountService вызывает CoreBanking-базу. Из таких связей собирается PurePath — сквозная трассировка в разделе Distributed Traces.
