> 📅 **День 1: Введение в систему Dynatrace** → Тема 9 из 14: «Построение базовых линий и работа с порогами»
...
**Как срабатывание выглядит в продукте.** В хронологии карточки проблемы платформа сама показывает механику оценкой скользящего окна; дословная строка с живого тенанта (сверено 2026-07-28): «Davis evaluates performance across timeframe (for example, 3 of 5 minutes show anomalous performance)». То есть аномалия фиксируется не по одиночному выбросу, а по доле аномальных минут в окне наблюдения: это прямая пришивка сегодняшней темы к завтрашнему разбору Problems.
...
1. Открыть **Settings → Anomaly detection → Services** (`/ui/settings/builtin:anomaly-detection.services`).
2. Найти блок **Reference period** и прочитать выставленный период эталона с экрана.

Что должно получиться: названный вслух период (значение экрана authoritative для этой сборки) и ответ на контрольный вопрос: «ночной бэкап каждый день грузит диск на полчаса: что сделает с ним baseline через неделю наблюдений?» (примет в норму для этого времени суток; будить дежурного перестанет).
