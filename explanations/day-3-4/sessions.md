> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 9 из 15: «Сессии пользователей: структура, атрибуты, Session Replay»
...
<!-- TODO(пре-флайт): скриншот ниже дублирует снимок Шага 1 (Session-List); собственный снимок replay-landing снять с живого тенанта при следующей съёмке -->
...
- **Masking modes.** Доступно четыре режима:
  - **Mask all**: по умолчанию максимально жёсткий, маскируется весь текст, ввод, атрибуты и картинки.
  - **Mask user input**: маскируется только пользовательский ввод.
  - **Allow list**: показываем только перечисленные элементы, остальное маскируется.
  - **Block list**: маскируем только перечисленные элементы, остальное видно.
  - Для отдельных элементов: атрибут `data-dtrum-mask` прямо в HTML.
...
### Два практических правила разбора заявок по сессиям

**Тегированный пользователь: предпосылка поиска по имени.** Поиск сессии по имени пользователя работает только потому, что приложение передаёт тег пользователя в агент (identify-вызов RUM). Без тега остаётся поиск по IP, браузеру и времени. На демо-тенанте живут тегированные пользователи (например, `maria`, `Tatjana Jonker`): по ним и строится показ. На своей инсталляции проверьте, что фронт передаёт user tag, иначе первая линия ищет вслепую.

**Ловушка часового пояса.** Жалоба формулируется во времени пользователя, а экран показывает время в поясе профиля. Правило: перед фильтрами явно проговорить, в чьём времени жалоба, и брать окно поиска с запасом в обе стороны. Пример арифметики: «вчера в 19:00» в Казахстане (UTC+5) это 14:00 UTC; ошибка на этом шаге даёт ложное «не воспроизводится».

---

## 📝 Практика (3 минуты, выполнимо на demo-тенанте)

1. Открыть список сессий (`/ui/user-sessions`), при пустом окне расширить таймфрейм.
2. Найти любую сессию с ошибками и открыть её.
3. Прочитать действия как сюжет и одной фразой сказать: что пользователь делал прямо перед ошибкой.

Что должно получиться: фраза вида «пользователь искал перелёт, на третьем действии словил JS-ошибку»: это и есть язык, на котором сессия разговаривает с первой линией.
