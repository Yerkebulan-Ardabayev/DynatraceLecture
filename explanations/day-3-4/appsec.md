...
- **Davis Security Score (DSS)**: уровень риска (Critical / High / Medium / Low / None). DSS берёт за основу CVSS (Common Vulnerability Scoring System) и пересчитывает его под ваше окружение: учитывает доступность процесса из интернета (public internet exposure) и наличие достижимых хранилищ данных (reachable data assets).
...
ЕСЛИ у CVE базовый CVSS = High, но затронутый сервис не доступен из интернета и не имеет достижимой БД → ТО Davis понижает балл (например, до Medium) и эта уязвимость уходит ниже в очереди. DSS никогда не поднимает оценку выше базового CVSS, он только понижает её или оставляет без изменений.

*Роль для compliance.* Регулятор требует отчёты по уязвимостям. DSS и `Actual exposure` помогают приоритизировать исправления: сначала то, что реально эксплуатируемо.
