> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 7 из 15: «Инфографика приложений: показатели UX и производительности»
...
- **Rage clicks**: пользователь многократно кликает в одно место, признак раздражения.
...
*Пример инцидента.* Жалоба «страница платежа тормозит». Инженер открывает карточку приложения (**Application Observability → Frontend** → `https://guu84124.live.dynatrace.com/ui/applications`, клик по имени приложения в колонке **Name**) и последовательно снимает три показателя:
...
*Вывод.* Дело не в скорости. Кнопка payment не срабатывает (JS-ошибка), пользователи кликают много раз, уходят без оплаты. Куда идти дальше: JS-ошибки видны в карточке приложения, отдельные визиты в **User sessions** (`https://guu84124.live.dynatrace.com/ui/user-sessions`), запись поведения через Session Replay (День 5, тема 4).
...
## 📝 Практика (2 минуты; состав живой карточки приложения сверяется на пре-флайте)

1. Открыть список приложений и карточку `www.angular.easytravel.com`.
2. Найти три типа действий (Load / XHR / Custom) и определить, какой из них сегодня самый медленный.

Что должно получиться: один из трёх типов, названный вслух, с числом с экрана. Контрольный вопрос себе: какой порог T для Apdex вы поставили бы внутреннему порталу и почему (внутренние системы обычно жёстче, публичные мягче).
