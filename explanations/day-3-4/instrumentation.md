> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 5 из 15: «Инструментирование приложений: Web, Mobile»
...
## 📝 Практика (3 минуты, обычный браузер + мониторимая страница)

Поймать JS-агента «с поличным» на любой странице, где включён Web RUM (на демо: `www.angular.easytravel.com`, доступность из учебной сети проверяется заранее):

1. Открыть страницу → F12 (DevTools) → вкладка **Network** → в фильтр набрать `ruxit` → обновить страницу (F5).
2. Прочитать имя загрузившегося файла агента.
3. Снять фильтр, покликать по странице и увидеть уходящие пачки событий (beacon-запросы).

Что должно получиться: живое доказательство, что агент это обычный JS-файл, отдаваемый вместе со страницей, а события уходят пачками по действиям, не потоком. Имя файла настраивается на странице настройки имени RUM-файла (Web → RUM JavaScript → File name, `/ui/settings/builtin:rum.web.rum-javascript-file-name`): корпоративные стандарты и блокировщики: главные причины его менять. Если сеть класса закрыта: показ по заранее снятым скриншотам вкладки Network.
