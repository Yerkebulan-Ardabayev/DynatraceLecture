...
- **Service-level availability**: отношение успешных вызовов сервиса к общему количеству. *Когда:* для сервиса целиком, когда важна общая доступность (типовой выбор для backend-API).
- **Service-method availability**: то же на уровне отдельного key request. *Когда:* когда обязательство завязано на конкретную операцию (`/auth/login`), а не на сервис целиком.
- **Service performance**: доля вызовов с откликом меньше порога. *Когда:* когда клиенту важна скорость, а не только факт ответа (платёж < 3 секунд).
- **User experience**: на базе Apdex. *Когда:* для фронтенда и RUM, где цель: удовлетворённость пользователя, а не сырой технический показатель.
- **Mobile crash-free users**: доля сессий без сбоев. *Когда:* для мобильного приложения, где главный риск: краши, а не время отклика.
- **Synthetic availability**: успешность Synthetic-чеков. *Когда:* когда доступность надо мерить извне синтетикой, независимо от реального трафика (ночью, в простое).
