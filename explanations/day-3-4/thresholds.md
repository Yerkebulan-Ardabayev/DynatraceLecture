> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 10 из 15: «Настройка порогов, аномалий и базовых линий (для apps)»
...
Каждый тип имеет два режима: **automated baselining** (Davis AI, искусственный интеллект Dynatrace, учится на реперном периоде: по умолчанию **последние 7 дней**) или **fixed thresholds** (фиксированные пороги) с тремя уровнями чувствительности: Low / Medium / High.

**Как порог решает, заводить ли Problem (на примере Failure rate, рост доли ошибок).** В режиме automated baselining вы задаёте два значения: относительный порог в **%** (на сколько процентов выросла доля ошибок) и абсолютный порог в **%** (выше какого значения доли ошибок это уже проблема). Логика срабатывания:

- ЕСЛИ нарушены **оба** порога сразу (и относительный, и абсолютный) → ТО Davis заводит Problem.
- ЕСЛИ нарушен только один из двух → ТО Davis молчит (это защита от ложных срабатываний).

Например (иллюстративные числа, не значения по умолчанию): относительный порог 50%, абсолютный 3%. Базовый failure rate приложения 2%. ЕСЛИ доля ошибок выросла до 3,5% (рост на 75%, это больше 50%, и само значение 3,5% больше 3%, нарушены оба) → ТО заводится Problem. ЕСЛИ выросла только до 2,8% (рост 40%, меньше 50%, относительный порог не нарушен) → ТО Problem нет. В режиме fixed thresholds для Failure rate задаётся уже один абсолютный порог в **%**, выше которого шлётся алерт.

**Что делает выбор Low / Medium / High (для fixed thresholds).** Уровень задаёт статистическую строгость, с которой нарушение порога превращается в алерт:

- ЕСЛИ выбран **Low** → ТО используется высокая статистическая уверенность: кратковременные нарушения (например, скачок нагрузки) Problem не заведут, дежурный получит меньше шума, но узнает о деградации позже.
- ЕСЛИ выбран **High** → ТО статистическая уверенность не применяется: **каждое** нарушение порога заводит алерт, реакция мгновенная, но ложных срабатываний больше.
- **Medium** посередине: разумный баланс, не алертит на каждое единичное нарушение.

*Критичность.* Страница: одна из ключевых для алертов по клиентским web-приложениям. Её настройка прямо определяет баланс «скорость реакции против шума» и насколько быстро дежурный узнает о проблеме.
...
**Как работает.** Dynatrace строит baseline числа крашей по версии приложения; чувствительность настраивается тремя уровнями: Low / Medium / High. Точные окна наблюдения и минимальные пороги трафика в публичной документации не зафиксированы.
