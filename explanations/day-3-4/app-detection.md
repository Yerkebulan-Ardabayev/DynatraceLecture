> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 6 из 15: «Детектирование приложений: правила, группы, лучшие практики»
...
**Правила матчатся по URL** приложения. Правила оцениваются **сверху вниз, срабатывает первое совпавшее**, дальнейшая обработка останавливается. Лимит: **до 1000 правил на окружение**.
...
## 📝 Практика (3 минуты, выполнимо на demo-тенанте + бумага)

1. На экране Application detection (`/ui/settings/builtin:rum.web.app-detection`) найти инструмент **Check your existing detection rules**, ввести адрес демо-приложения `www.angular.easytravel.com` (имя приложения демо-тенанта) и посмотреть, какое правило сработало.
2. На бумаге написать правило для своего главного домена: что в поле URL, какое имя приложения.

Что должно получиться: пара «URL → имя приложения» для своей инсталляции и устный ответ, на сколько приложений вы разбили бы свой портал и по какому признаку (домены, команды, контуры).
