> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 2 из 15: «Связи сервисов и Service Flow»
...
**Соседние кнопки той же секции (сверено на живом тенанте 2026-07-28).** Рядом с **View service flow** в блоке Understand dependencies живут **View backtrace**, **View distributed traces**, **View web requests**, **View related logs**: один блок карточки закрывает все направления анализа зависимостей.

**Backtrace: обратный Service Flow.** Кнопка **View backtrace** отвечает на зеркальный вопрос: не «кого сервис вызывает», а «кто вызывал его». Дерево читается от текущего сервиса вверх на 1-2 уровня: сервис → кто его вызвал → кто вызвал того. Типовой сценарий: в Root cause проблемы всплыл незнакомый сервис; View backtrace показывает, чьи запросы через него идут, и незнакомец получает контекст (чей он, какой фронт его дергает).
...
1. Открыть карточку `easyTravel Customer Frontend` → блок **Understand dependencies** → **View service flow**.
2. Найти самое широкое ребро (самый частый путь вызовов) и навести курсор: число вызовов, время, ошибки.
3. Вернуться и нажать **View backtrace**: прочитать, кто вызывает сервис, на 1-2 уровня вверх.

Что должно получиться: устный ответ «кого сервис вызывает чаще всего и кто чаще всего вызывает его»; это и есть пара Service flow / Backtrace.
