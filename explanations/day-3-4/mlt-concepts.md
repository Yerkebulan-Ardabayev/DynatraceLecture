> 📅 **День 3-4: Архитектура сквозного мониторинга и Observability** → Тема 1 из 15: «Метрики, логи, трейсы: концепции и реализация»
...
Observability (наблюдаемость) стоит на трёх типах данных: метрики (что происходит, в числах), логи (что записала система, текстом), трейсы (как запрос прошёл через сервисы). По отдельности каждый отвечает только на свой вопрос: метрика говорит «response time вырос до 500 мс», но не говорит, где именно; лог показывает ошибку, но не показывает всю цепочку вызова. Сила Dynatrace в том, что все три привязаны к одной сущности (сервис / процесс / хост), поэтому из графика метрики переход в её логи и трейсы делается одним кликом, без ручной корреляции.

Дальше по теме: сначала пять экранов, где эти три типа данных живут в Managed (КАРТА ниже), затем как они связаны в единую модель и сколько каждый тип хранится в air-gapped кластере.
...
**Как читать открытую трассировку (формула зон экрана).** Слева: дерево вызовов, читается сверху вниз (вход → сервисы → база). Справа: таймлайн запроса: где время «размазано» по многим шагам, а где съедено одним. Строки с ошибками подсвечены: по ним и спускаемся. Точную цветовую легенду читайте на самом экране вашей сборки. Второй заход по той же формуле занимает уже пару минут: зоны экрана те же на любом трейсе.
...
- 64-bit Go-исполняемые файлы: автоматическая инъекция инструментации в бинарь.
...
- **Log Monitoring Classic.** Elasticsearch с replication factor 2. Срок хранения: настраивается, максимум **90 дней**.
...
Зачем L1/L2 эти сроки на практике:
- ЕСЛИ инцидент старше настроенного срока хранения логов (максимум 90 дней) → ТО лог-строки по нему уже удалены, расследовать придётся по метрикам и Davis-событиям, не по логам.
- ЕСЛИ нужен полный PurePath конкретного запроса месячной давности → ТО он ещё доступен (трейсы до 365 дней), но детальный код-профиль уже нет (Code-Level Insights только 10 дней): для разбора кода реагировать в первые дни.
- ЕСЛИ строите график за период старше 400 дней → ТО точки будут с шагом 1 день, а не 1 минута (метрики старше 400 дней прорежены до суточной гранулярности), пики внутри суток на таком масштабе уже не видны.
...
1. Открыть **Application Observability → Distributed Traces** (`/ui/diagnostictools/purepaths`).
2. Отфильтровать трейсы с ошибкой и открыть любой из них.
3. Прочитать его по формуле зон: дерево слева сверху вниз, таймлайн справа, спуститься к подсвеченной строке с ошибкой.

Что должно получиться: имя сервиса, на котором ошибка загорелась. Это тот же навык, что в практике карточки сервиса (День 2), но со стороны общего списка трейсов.
