...
Экран показывает все БД, к которым обращаются сервисы под мониторингом. Сверху: заголовок, на снимке этой темы **30 Databases** (у вашего тенанта число другое). Это количество баз на демо-тенанте за выбранный период. На боевой цифра зависит от числа инструментированных приложений и их трафика.
...
**Карточка БД: как открыть.** Где: этот же экран Databases, **Application Observability → Database Services** → `https://guu84124.live.dynatrace.com/ui/databases`. Что нажать: имя базы в колонке **Name**, либо кнопку **Open service drilldown** в той же строке. Для тренировки берите реальную строку с демо: `[eks-live][easytrade-live-debugger] TradeManagement`. Как вернуться: кнопка «назад» браузера либо снова **Database Services**.

**Что внутри карточки БД:**
