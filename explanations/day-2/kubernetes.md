...
*Решение (с поправкой на air-gapped).* Настройка Security Posture Management в Settings существует, но результатов в air-gapped Managed Classic она не даёт (KSPM живёт в новой Apps-платформе, см. Шаг 3 выше). Поэтому квартальный отчёт в закрытом контуре собирается сторонними инструментами (kube-bench, Trivy, Falco и аналоги), а Dynatrace в этом процессе закрывает инвентаризацию: какие кластеры, ноды, workload'ы и версии реально работают. DevOps устраняет findings между квартальными ревью.
