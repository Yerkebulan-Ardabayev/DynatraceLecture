...
## 🧭 Что такое мониторинг ОС и зачем он нужен

Это первая тема Дня 2, поэтому начнём с контекста, а уже потом перейдём к экранам тенанта.

Мониторинг ОС (операционной системы), это сбор базовых метрик каждой машины с установленным OneAgent: загрузка процессора (CPU), оперативная память (RAM), дисковый ввод-вывод (IO, чтение и запись на диск), сетевой трафик, запущенные процессы и сервисы ОС (Windows-службы и Linux systemd units). OneAgent собирает их автоматически сразу после установки, отдельно настраивать сбор не нужно.

Зачем это L1/L2-инженеру: когда приложение тормозит или падает, причина часто лежит ниже уровня кода, на уровне хоста. Диск забит под завязку, память ушла в swap, один процесс съел все ядра CPU, упала критичная служба СУБД. Без метрик хоста такую причину видно только по косвенным симптомам. С метриками хоста виден сам корень: какой ресурс кончился, на какой машине, в какую минуту.

Что настраивается в этой теме на живом тенанте: список хостов и карточка хоста (где смотреть метрики), мониторинг сервисов ОС (какие службы считать критичными), глобальные опции дисков и пользовательские правила аномалий по дискам (когда заводить Problem по свободному месту). Дальше по Дню 2 идут процессы, контейнеры, базы данных и сети, мониторинг ОС, это их фундамент.
...
Страница: глобальные параметры работы с дисками на всех хостах. Заголовков третьего уровня на странице нет, она состоит из тумблеров и значений. Хлебная крошка: Settings → Preferences → Disk options.
...
*Типовой случай.* Диск под бэкапы СУБД, 10 ТБ. Бэкапы пишутся пачками и занимают 60-70% диска. ЕСЛИ оставить глобальное правило «меньше 30% свободного, это проблема» → ТО оно срабатывает каждую ночь на штатных бэкапах и генерирует шум. ЕСЛИ задать кастомное правило «на диске `/backup` на хостах `db-*` считать проблемой только ниже 5%» → ТО ночные всплески не триггерят алерты, а реальное переполнение всё равно ловится.
