...
В captured тенанте видно **239 Services** (h2 `239Services`). Эта страница используется как точка входа в детальный анализ — клик по имени сервиса открывает его карточку, где и происходит вся работа, описанная в этой теме.
...
*Что на странице.* Заголовков в captured нет, страница — тумблеры и правила.
...
**Percentiles (90 / 95 / 99).** Считаются по алгоритму t-digest. Позволяет агрегировать без хранения полной истории запросов. Компромисс: точные перцентили требуют слишком много памяти, приближённые достаточны для практики.
