...
Страница: настройки аномалий для **Disk Edge**, мониторинга дисков через Edge-сенсоры (не OneAgent, а опрос с ActiveGate с соответствующей ролью). Заголовков на странице нет, она минималистичная.
...
На демо-тенанте правил нет. Для демо без реального оборудования это нормально. Страница используется, когда в контуре есть дисковые массивы с мониторингом через ActiveGate.
...
2. В Dynatrace Hub ищется нужный extension, `.zip` скачивается на машине с интернетом и загружается в кластер через Hub → Upload extension.
...
- **RUM: User action data**: конфигурируется, **до 365 дней** максимум.
- **RUM: User sessions**: **35 дней (фикс)**.
- **RUM: Mobile crashes**: **35 дней (фикс)**.
- **RUM: Session Replay**: конфигурируется, **до 35 дней** максимум.
- **Log Monitoring Classic**: конфигурируется, **до 90 дней** максимум. Хранятся в Elasticsearch в зоне кластера, размер дискового хранилища задаётся лицензией.
