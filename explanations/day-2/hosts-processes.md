...
Термины темы: `Host / хост`, `Process / процесс` (конкретный запущенный бинарник с PID), `Process Group / группа процессов / PG` (логическое объединение одинаковых процессов одного приложения), `Process Instance / инстанс процесса`, `Smartscape / карта топологии`, `Service / сервис / логическая backend-единица`.
...
**Структура правила (два вида по документации):**

- **Simple-правило**: только разделение (split) группы по значению environment variable или Java system property. Самый частый случай.
- **Advanced-правило**: разделение или объединение (split/merge) по детектированным свойствам процесса, для нестандартных случаев.

**Типовые правила:**

- Split по environment variable `APP_ENV`: разделяет одноимённые приложения в dev / test / prod.
- Split по `DT_CLUSTER_ID`: классический способ развести инсталляции одного приложения, в том числе в Kubernetes.
- Split по Java system property: разные инсталляции одного Java-приложения в отдельные Process Groups.
...
**ЕСЛИ → ТО: как условие правила меняет поведение алертинга.** Поле Condition это прямой рычаг: что именно записали, так Davis и поведёт себя при пропаже процесса.

- ЕСЛИ не задать ни одного правила → ТО действует дефолт: процесс сервиса исчез (был минуту назад, сейчас нет) → Problem создаётся сразу.
- ЕСЛИ в Condition выставить «процесс не обнаружен N секунд» с порогом 60 → ТО короткие плановые рестарты (меньше 60 секунд) Problem не порождают, а реальное падение (отсутствие дольше 60 секунд) заведёт Problem.
- ЕСЛИ в Action указать Alerting Profile → ТО уведомление уйдёт только адресатам этого профиля, иначе Problem появится на экране, но никого не оповестит.
...
**ЕСЛИ → ТО: тумблер enabled и квота 60 минут.**

- ЕСЛИ тумблер enabled выключен → ТО детальные снимки по этому процессу не собираются вовсе, в диагностике останутся только базовые хостовые метрики.
- ЕСЛИ процесс превысил 1% CPU, памяти или сети при включённом тумблере → ТО снимок (20 минут: 10 до и 10 после триггера) пишется автоматически, без участия оператора.
- ЕСЛИ за сутки триггеры уже выбрали 60 минут метрик на этом хосте → ТО следующий триггер в этот день снимок не создаст, придётся ждать суток.
...
- **Baseline не формируется.** Каждая новая Process Group: новая модель Davis AI. Ей нужно 7 дней истории для baseline. ЕСЛИ на экране Process grouping rules (Шаг 2) задать стабильное правило (Merge по имени deployment вместо случайных имён подов) → ТО Process Group живёт дольше недели, Davis накапливает baseline и ловит аномалии. ЕСЛИ правила плохие и новая группа рождается раз в день → ТО baseline не успевает сформироваться, аномалии не детектируются.
