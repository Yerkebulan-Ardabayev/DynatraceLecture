...
Если нажал «Разрешить»: приложение само ставит согласительный cookie (имя выбирает приложение, в доке пример `sessionReplayConsent`) и вызывает opt-in API `dtrum.enableSessionReplay()`; запись начинается после этого вызова. На снятой странице Session Replay cookie в Settings один тумблер: полей имени cookie, домена и пути там нет (`dtCookie` это общий служебный RUM-cookie, не настройка этой страницы).
...
- **Allow list**: маскируется всё, как в Mask all, кроме явно перечисленных селекторов (перечисленное записывается открыто). Дока рекомендует этот режим для большинства приложений.
- **Block list**: записывается всё, маскируются только элементы из списка (стартово список предзаполнен правилами Mask all, его можно очистить и дополнить).
