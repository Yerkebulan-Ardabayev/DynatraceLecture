...
- **Rate limit**: ограничение частоты запросов; точная цифра «N в минуту» в /managed/-документации одной таблицей не зафиксирована, при превышении API отдаёт HTTP 429.
...
## 📝 Практика (2 минуты, на бумаге)

Записать одной строкой сценарий для своей инсталляции: «какая ручная операция у нас повторяется так часто, что её первой стоит перевести на API» (выгрузка метрик в отчёт, pull-опрос проблем в мессенджер, накатка дашбордов из Git).

Что должно получиться: одна строка вида «операция → какой API endpoint → какой scope токена». Правило гигиены из темы: один сценарий = один токен с минимальными scope.
