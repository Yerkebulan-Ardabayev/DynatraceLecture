...
**Карточка приложения: как открыть.** Где: этот же экран Applications (`https://guu84124.live.dynatrace.com/ui/applications`). Что нажать: имя приложения в колонке **Name**. Куда попадаешь: карточка одного приложения, внутри графики, список user actions по типам, топ проблем и география пользователей. Как вернуться: кнопка «назад» браузера либо снова экран Applications. Разбор содержимого карточки: День 3-4, тема 7 «Инфографика приложений».
...
*Что настраивает.* На снятой env-странице только тумблеры; сами пороги задаются на страницах Configure user experience score конкретного приложения:
...
**Типы action, для которых существуют пороги (задаются в настройках приложения):**
...
*Что настраивает.* На снятой странице один тумблер включения аналитики раздражителей. Сами понятия:

- **Rage clicks**: серия быстрых кликов по одному элементу. Означает «жму, ничего не происходит», типично на «зависшей» кнопке. Точные пороги детекции в публичной доке не зафиксированы.
- **Conversions**: достижение бизнес-цели (покупка, регистрация, отправка заявки). Настраивается в самом приложении: Behavior analytics → Conversion goals.
