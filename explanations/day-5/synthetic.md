...
⚠️ **На демо-тенанте `guu84124` страница `/ui/synthetic` возвращает 403.** Учебный gap: у реального клиента Managed доступ будет. Разбор по общему знанию платформы плюс доступные снятые настройки.
...
Логика выбора частоты: ЕСЛИ проверка дешёвая (HTTP health endpoint) → ТО ставите её часто (каждую минуту), нужное падение Davis заметит почти сразу. ЕСЛИ проверка дорогая (browser clickpath с реальным браузером) → ТО частоту снижаете (каждые 15-30 минут), иначе ActiveGate перегрузится, ведь потолок все равно 1 запуск в минуту на монитор (см. «Ограничения» выше). Поэтому общая нагрузка от такого набора умеренная, один ActiveGate справляется.

### Почему 403 на демо-тенанте
...
На боевом Managed-тенанте, где модуль активирован, доступ настраивается через IAM на уровне Management zones и Permissions.
