...
**Лайфхак входа без написания кода (сверено на живом тенанте 2026-07-28).** На экране Session Segmentation (`/ui/user-sessions`) накликать знакомые фильтры (браузер, страна), затем нажать ссылку **Query User Sessions**: она ведёт на `/ui/user-sessions/query`, и запрос из накликанных фильтров уже готов: его не писали, его можно прочитать по-русски и упростить. Синтаксис учится по живым примерам быстрее, чем по справочнику.

**Сохранение запроса как метрики.** Полезный USQL-запрос превращается в кастомную метрику сессий: экран User session custom metrics, `/ui/settings/builtin:custom-metrics` (схема подтверждена документацией Managed; кнопка сохранения на самом экране запроса на живом тенанте не сверялась). Дальше метрика живёт как обычная: график в Data Explorer, порог в Metric events, плитка на дашборде.
...
По формуле Apdex из этого распределения получилось бы: (203 + 0.5 × 52) / 278 = 0.824. Важно: запрос считает session-level User experience score, а Apdex в карточке приложения это action-level метрика с другими порогами, поэтому числа могут не совпасть. Ценность в другом: распределение мы получили сами SQL-запросом и можем комбинировать как хотим.
...
1. Открыть **Query User Sessions** (`/ui/user-sessions/query`), при желании войти лайфхаком: фильтры на Session Segmentation → ссылка Query User Sessions.
2. Выполнить запрос «топ стран по числу сессий за сегодня» (образец: разбивка по странам выше, укоротить под себя: SELECT country, count(*) FROM usersession GROUP BY country ORDER BY count(*) DESC).
3. Нажать **Run query** и прочитать таблицу.

Что должно получиться: выполненный запрос и топ-строка таблицы. Антипаттерн, который практика прививает не делать: SELECT * без LIMIT на большом окне вешает отрисовку таблицы.
