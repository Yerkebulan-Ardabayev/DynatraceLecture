...
- **Режим подавления**: подавить только уведомления, оставив обнаружение проблем, либо полностью отключить детекцию на время окна (тогда проблемы за окно не отображаются вовсе).
...
*Живое состояние экрана (сверено на живом тенанте 2026-07-28).* Классический экран Metric events жив (на демо: под SaaS-баннером, который вслух не читаем). Колонки списка конфигураций: **Enabled / Summary / Query type / Model type / Management zone**. Блок **Overview of limits** показывает исчерпание лимитов по типам модели тремя строками вида «Static threshold: N out of 100», «Auto adaptive threshold: N out of 100», «Seasonal baseline: N out of 100» (значения прочитаны с экрана демо-тенанта; на вашей сборке сверяйте по своему блоку Overview of limits). Это UI-квоты по типам модели; документированный общий лимит конфигураций: см. «Лимит» ниже.
...
Davis, это AI-движок Dynatrace. Он:

1. Считывает метрики в реальном времени.
2. Строит baseline (референсное значение) для каждой метрики автоматически (референсный период: 7 дней).
3. Детектит статистически значимые отклонения от baseline.
...
## 📝 Практика (3 минуты, на бумаге)

Спроектировать свой первый кастомный алерт тремя строками: метрика (из Data Explorer, с историей), порог (по истории метрики, не с потолка), получатель (какой alerting profile и какой канал доставки).

Что должно получиться: три строки, готовые к переносу в формы «событие → профиль → интеграция». Единый нейминг на всех трёх экранах (например, одно и то же имя алерта в metric event, профиле и интеграции) сильно упрощает сопровождение цепочки.
