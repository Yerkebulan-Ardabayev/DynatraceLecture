...
> 💡 **Зачем эта тема (первая в Дне 5).** До сих пор мы смотрели на систему со стороны сервера: OneAgent внутри JVM или процесса меряет, сколько работал бэкенд. Но пользователь видит не это, а полное время от клика до картинки на экране, куда входят сеть, браузер и рендеринг. Real User Monitoring (RUM, мониторинг реальных пользователей) добавляет вторую точку наблюдения: агент в браузере или мобильном приложении. ЕСЛИ собирать данные только серверным агентом → ТО видно лишь время обработки на сервере (например, 35 мс), а реальное ожидание пользователя (например, 730 мс) и его причина (медленная сеть, тяжёлый JavaScript) останутся невидимыми. В этой теме разберём четыре экрана RUM на тенанте и устройство механизма изнутри.
...
**Правая часть: таблица сессий.** Колонки со снимка: User session, Replay, User, User experience score, Duration, User action count, Total conversions, Errors and annoyances.

**Карточка сессии: как открыть.** Где: этот же экран User sessions (`https://guu84124.live.dynatrace.com/ui/user-sessions`). Что нажать: строку в таблице справа, колонка **User session**. Куда попадаешь: карточка одного визита с полным списком user actions в хронологическом порядке. Как вернуться: кнопка «назад» браузера либо снова экран User sessions. Разбор содержимого карточки: День 3-4, тема 9 «Сессии пользователей».
...
- **Android.** Подключается **Dynatrace Android Gradle plugin** (опубликован в Maven Central) на уровне top-level `build.gradle` / `build.gradle.kts`.
  - *Как работает.* Bytecode instrumentation (Java / Kotlin и другие JVM-языки): plugin обрабатывает source-файлы основного модуля и сторонних библиотек до R8-обфускации.
  - *Что не покрывается автоматически.* Native-код, web-компоненты и resource-файлы (XML layouts).
- **iOS.** Рекомендуемый способ интеграции: **Swift Package Manager** (репозиторий `https://github.com/Dynatrace/swift-mobile-sdk.git`, добавляется в Xcode через `File → Swift Packages → Add Package Dependency`).
  - *CocoaPods.* Поддерживался исторически, но новые версии OneAgent SDK там больше не публикуются: для хотфиксов рекомендуется миграция на SPM.
  - *Static builds и Carthage.* Перестали поддерживаться, начиная с версии 8.323.
...
Новая session с тем же пользователем считается отдельной: у неё свой session-id. ЕСЛИ пользователь сделал паузу дольше таймаута бездействия (Web 30 минут, Mobile 10 минут) и вернулся → ТО Dynatrace закроет прежнюю сессию и заведёт новую с новым session-id, а не продолжит старую. Только что закрывшаяся сессия может оставаться в UI ещё какое-то время до полной финализации.
...
На всех страницах тенанта в верхней панели виден значок `278` рядом с кнопкой пользователя. Это количество активных проблем, зарегистрированных Davis AI в текущий момент. Счётчик живой, обновляется в реальном времени, поэтому у вас будет другое число. Что нажать: само число. Куда попадаешь: экран Problems, **Observe and explore → Problems** → `https://guu84124.live.dynatrace.com/ui/problems`. Это не RUM-специфично: счётчик виден на любой странице.
