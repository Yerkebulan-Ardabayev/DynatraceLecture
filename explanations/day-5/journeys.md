...
**Типовой сценарий «пользователи не доходят до оплаты»: маршрут по шагам.**

1. **User sessions.** Открываю `https://guu84124.live.dynatrace.com/ui/user-sessions`.
2. **Сужаю выборку.** В левой колонке ставлю **Conversions and bounces** = `Converted: No` и **Application type** = `Web`.
   *Что вижу:* только сессии, где пользователь до цели не дошёл.
3. **Открываю одну сессию.** Клик по строке в колонке **User session**.
   *Что вижу:* хронологию user actions этого визита.
4. **Читаю хронологию.** Пример реального разбора: пользователь открыл главную, выбрал товар, добавил в корзину, пошёл на checkout, 4 раза прожал кнопку «Оплатить», получил rage click-и, закрыл вкладку.
   *Итог:* точка боли найдена не по метрике в среднем, а по конкретному визиту.
...
- ЕСЛИ задать условие сегмента (например, `Conversions and bounces` = `Converted: No` плюс `Application type: Web`) → в списке останутся только подходящие сессии и нужную видно сразу; ЕСЛИ фильтр не задавать → отдаётся весь трафик (список показывает только первые сессии по времени старта), и нужную теряешь в общей массе.
...
Два представления одного и того же факта: «система, это сеть, где узлы связаны рёбрами трафика».
