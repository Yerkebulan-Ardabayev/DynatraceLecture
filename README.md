...
# -> output/training_v2.html (44 карточки лектора, ~1.5 MB, офлайн: 0 сетевых запросов)
```

**Что добавляет v2:**
- **Палитра Ctrl+K** — нечёткий поиск по 44 темам плюс полнотекст по контенту с подсветкой и прыжком к совпадению; Esc возвращает к теме плана.
...
По одной на каждую из 44 тем. YAML-шапка (`topic_id`, `day_id`, `timing_min`, `verified`) плюс 6 блоков: `## ГДЕ` (route и путь) / `## ЗАЧЕМ` / `## ЦИФРЫ` / `## ЕСЛИ→ТО` / `## ЗАПАСНОЙ ПЛАН` / `## ВОПРОСЫ АУДИТОРИИ`.
